...
Für die Darstellung von nicht diskreten Zeit sowie Zustandsräumen wurden komplexere Versionen der Markovprozesse entwickelt. Insbesondere wenn es sich um eine Bewertung mehrdimensionaler Systeme handelt, reichen einfache diskrete Markovketten oftmals nicht aus.
Speziell wenn eine Linearität der zugrundeliegenden Funktionen sowie eine gauss'sche Verteilung der Messfehler angenommen werden kann, bietet sich der Einsatz von Kalman Filtern an. Diese wurden 1960 von R.E. Kalman vorgestellt \citet{kalman1960new} und sind eine Methode zur Schätzung des Zustands eines dynamischen Systems aus einer Reihe von verrauschten Messungen.
Kalman Filter können aus einem vorherigen Zustand eine Vorhersage des nächsten Zustands treffen und diese Vorhersage dann mit einer neuen Messung aktualisieren, um eine verbesserte Schätzung zu erhalten. Dies geschieht durch die Anwendung von zwei Hauptschritten: dem Vorhersageschritt und dem Aktualisierungsschritt.

Der Kalman Filter modelliert das System als lineares Dynamisches System. Hierbei wird der Zustand $x_k$ zum Zeitpunkt $k$ durch die Zustandsübergangsmatrix $A$, die Steuerungsmatrix $B$ und das Steuerungssignal $u_{k-1}$ beschrieben. Der Prozessrauschen $w_{k-1}$ wird als Gaußsches Rauschen mit mittlerem Wert null und Kovarianz $Q$ modelliert.
...
Dabei kommt die Qualität der Schätzung darauf an, ob zu Beginn sinnvolle Kovarianzen für das gaußsche Rauschen, das die Messung beeinflusst, gewählt wurde, sowie ob die Matrix A die Dynamik des Systems gut beschreibt.
Der Kalman Filter kann auch über mehrere Dimensionen erweitert werden, um komplexere Systeme zu modellieren. In diesem Fall werden die Zustandsvektoren, Messvektoren und Rauschkovarianzmatrizen entsprechend angepasst.
Ein allgemeines Beispiel für ein Modell solches System ist:

\begin{equation}
x_{k} = H x_{k} + v_{k}
\end{equation}

Auch eine Erweiterung um Steuerungssignale ist möglich:

\begin{equation}
x_{k} = H x_{k-1} + B u_{k-1} + v_{k-1}
\end{equation}

Hier wird $x_k$, der Zustand des Systems zum Zeitpunkt $k$ aus der vorherigen Zustand $x_{k-1}$, dem Steuerungssignal $u_{k-1}$ und dem Prozessrauschen $w_{k-1}$ berechnet. Die Matrix $H$ beschreibt die Dynamik des Systems und $B$ beschreibt die Auswirkungen des Steuerungssignals auf den Zustand.
In diesem Beispiel gibt es also zwei Messungen und ein modelliertes Rauschen, die in das System einfließen: Das Steuerungssignal $u_{k-1}$, die Messung $x_{k-1}$ und das Prozessrauschen $w_{k-1}$.

Bei der Evaluation des Systems zum Zeitpunkt $x_k$ wird im ersten Schritt die Messung $z_k$ durch die Messmatrix $H$ und das Messrauschen $v_k$ beschrieben:
