...
einer der häufigsten und tödlichsten Krebsarten.\citep{labianca2010-colon-cancer}
...
Hidden Markov Models beinhalten, wie im vorigen Abschnitt erläutert, die Annahme, dass aufgrund des vorherigen versteckten Zustands das System vorhergesagt werden kann. 
Hierfür müsste das System durch die Menge der versteckten Zustände vollständig beobachtbar und adäquat abbildbar sein.
Ein Conditional Random Field erlaubt es also, die Annahme der vollständigen Beobachtbarkeit sowie die iid Annahme zu beweisen.
...
Bei der Betrachtung von statistischen Verteilungen gibt es insbesondere bei divergenten Systemen ein häufig aufkommendes Phänomen. Potenzgesetze beschreiben Systeme, denen sich selbst verstärkende Verteilungen zugrundeliegen. Deshalb kommt es oft dazu, dass
Die Modellierung von Systemen mithilfe von Markovketten obliegt in bestimmten Fällen eines Potenzgesetzes. \cite{10.1098/rsos.220531} In diesem Fall ist es wahrscheinlich, dass die Systeme keinen kontinuierlichen Zustand erreichen. Das kann Probleme verursachen, zum Beispiel wenn die Kombination der Wahrscheinlichkeiten zu einer stationären Verteilung führen sollte. Beispielsweise wenn durch die Simulation das wahrscheinlichste System verhergesagt werden soll, das
...
Jürgen Schmidhuber schlug bereits 1990 vor, die Welt, innerhalb derer ein neuronales Netzwerk operiert, durch zwei interagierende Netzwerke zu modellieren: Ein Modell-Netzwerk, das die Dynamik der Umgebung simuliert und vorhersagt, und ein Controller-Netzwerk, das Aktionen ausführt. Die zentrale Idee war es, die Umgebung durch das Modellnetzwerk \enquote{differenzierbar} zu machen, sodass der Fehler der Vorhersage genutzt werden kann, um das System mittels Gradientenabstieg zu optimieren \cite{schmidhuber1990diff}.

Das Problem der temporalen Analyse von Endoskopievideos kann als ein solches System betrachtet werden. Hierbei sind die Frames des Videos die messbaren, hochdimensionalen Eigenschaften des Systems (Observationen). Diese müssen in kompakte Zustände überführt werden, die den aktuellen Status des Endoskops im Darm abbilden. Daraus können Wahrscheinlichkeiten abgeleitet werden, wie sich das Endoskop im nächsten Zeitschritt verhalten wird oder welcher anatomische Abschnitt als nächstes sichtbar wird.

Modernere Ansätze, wie die von \cite{ha2018worldmodels} vorgestellten \enquote{World Models}, adressieren die Schwierigkeit, hochdimensionale visuellen Daten (wie Videoframes) modellieren. Sie unterteilen das Problem in drei Komponenten, die analog zur kognitiven Verarbeitung des Menschen gesehen werden können:

\begin{enumerate}
    \item \textbf{Vision Model:} Ein variationaler Autoencoder (VAE), der die hochdimensionalen Eingabebilder $x_t$ in einen niedrigdimensionalen, latenten Vektor $z_t$ komprimiert. Dies abstrahiert die visuellen Details und extrahiert die wesentlichen Merkmale des aktuellen Frames.
    \item \textbf{Memory Model (M):} Ein rekkurrentes neuronales Netzwerk (RNN), oft implementiert als LSTM oder MDN-RNN (Mixture Density Network), das auf der Basis der vergangenen latenten Zustände und Aktionen den zukünftigen latenten Zustand $z_{t+1}$ vorhersagt. Dies entspricht dem eigentlichen Weltmodell, das die Dynamik und Zeitabhängigkeit lernt.
    \item \textbf{Controller (C):} Ein einfaches Netzwerk, das basierend auf der aktuellen Sicht ($z_t$) und der Erinnerung ($h_t$ aus dem RNN) Entscheidungen trifft.
\end{enumerate}

Für die Segmentierung von Endoskopievideos ist insbesondere das Zusammenspiel von V und M relevant. Während das V-Modell die räumlichen Merkmale eines einzelnen Bildes (z.\,B. Textur eines Polypen) lernt, lernt das M-Modell die zeitliche Kohärenz (z.\,B. dass ein Polyp über mehrere Frames hinweg sichtbar bleibt oder dass nach dem Zökum das Colon ascendens folgt).

Eine Weiterentwicklung des Worldmodel Konzepts stellt die Dreamerarchitektur von Hafner et al. \citet{hafner2019dreamer} dar. Anstatt zukünftige Pixel aus den beobachteten Pixeln vorherzusagen (was rechenintensiv und fehleranfällig ist), lernen diese Modelle die Dynamik ausschließlich im latenten Raum. Statt wie bei Schmidhuber Pixelzustände direkt zu propagieren, lernen Dreamer-Modelle eine stochastische Übergangsdynamik $p_\theta(s_t \mid s_{t-1}, a_{t-1})$ im latenten Raum, sodass die zeitliche Entwicklung durch kompakte Zustände und deren rekursive Aktualisierung beschrieben wird. Während die Modellanpassung weiterhin über die Erklärung der Beobachtungen erfolgt (z.\,B. Rekonstruktion/Prädiktion), wird die eigentliche Sequenzplanung und Politikenoptimierung in Dreamer überwiegend im latenten Raum durchgeführt, was Rechenaufwand reduziert und die Optimierung stabilisiert.


Eine ähnliche Zielsetzung kann auf das Markov Switch State Modell angewandt werden. Ein Modell, das gelernt hat, wie sich die Anatomie im latenten Raum verändert, ist robuster gegenüber Rauschen (z.\,B. Unschärfe, Spiegelungen oder Verschmutzungen der Linse), da es nicht versucht, jedes Pixel exakt zu rekonstruieren, sondern den semantischen Inhalt der Szene über die Zeit zu verfolgen. Das Endoskopieszenario lässt sich somit als partiell observierbarer Markovprozess beschreiben. Der Arzt (oder der Algorithmus) sieht nie den gesamten Darm (Zustand $S_t$), sondern nur einen Ausschnitt (Beobachtung $O_t$). Ein World Model ermöglicht es, aus der Historie der Beobachtungen eine interne Repräsentation des globalen Zustands aufzubauen und somit die Segmentierung einzelner Frames durch den zeitlichen Kontext zu stabilisieren. Als Merkmal stabiler World Models kann verstanden werden, dass auch das Generieren realistischer Daten möglich wird.
