...
%*****************************************
Wie in den vorangegangenen Kapiteln dargelegt, besteht ein valides und belegbares Interesse daran, die Durchführung von Endoskopien durch den Einsatz von künstlicher Intelligenz und Informationssystemen effizienter zu gestalten. Dabei ist es jedoch, wie Hann et al. \cite{hann_entscheidungsunterstutzung_2025} darlegten, sinnvoll zu evaluieren wo sich die erweiterten Methoden tatsächlich positiv auf den Arbeitsaufwand auswirken. Um automatisiert assistierte Dokumentation von Endoskopievideos im Arbeitsalltag zu integrieren, sollte die Verarbeitung der Videodaten mit möglichst geringem Rechenaufwand erfolgen. Daher ist es reizvoll, effiziente Algorithmen zu entwickeln, die eine schnelle Segmentierung von Videoframes in Kategorien zu ermöglichen. Wenn diese Kategorien nach der Untersuchung bereits gesichert analysiert wurden, könnten Statistiken zur Endoskopie sowie vorgefertigte Dokumentationsskelette zur verfügung gestellt werden.
...
Am Ende soll vermieden werden, das die Berechnungen des Systems zu komplex für eine praxisrelevante Nutzung werden. Aus diesem Grund verwendet das System den Ansatz der Teilung entlang der temporalen Achse direkt in der Architektur.
...
Hierfür wird die folgende Hypothese aufgestellt: \enquote{Mit dem Einsatz eines HMM oder CRF zur Vorverarbeitung des Videos kann der Einsatz von CNN reduziert werden um ein effizienteres System zu erhalten. Dabei soll erprobt werden, ob für die weitere Verarbeitung irrelevante Zustände sowie Regionen von erhöhtem Interesse unter zuhilfenahme eines diskriminativen (CRF mit logistischer Regression) oder generativen Modells (HMM mit naiver Bayes klassifikation) schneller und zuverlässiger identifiziert werden können als mit einem CNN alleine.}
...
Da es sich um ein stochastisches Modell handelt, wird das Gesetz der totalen Wahrscheinlichkeit als Grundlage für die Schätzung einer Zufallsvariable angenommen. 
...
\]
Dieses übergeordnete Markov-Modell bestimmt, welcher Kalman-Filter zur jeweiligen Zeit wie stark aktiv ist und welche Systemdynamik angenommen wird.
...
\subsection{CRF - ähnliches Verhalten einer Bank von Kalman Filtern}

In der Umsetzung sollen für jeden Zustandsübergang Kalmanfilter auf einem Featurevektor trainiert werden. Die Reihenfolge der Auswertung des Kalmanfilters wird durch die Zustandsübergangswahrscheinlichkeiten der Markovkette bestimmt. Da die Kalmanfilter vorher auf allen Frames, die mit dem gegebenen Zustand annotiert sind, trainiert werden um eine individuelle Binäre Klassifikation durchzuführen, erfüllen Sie das Markovkriterium nur teilweise.
Sie nutzen die Informationen der vorherigen Frames nicht direkt sondern über das Training. Deshalb bewerten Sie den Übergang anhand des gelernten Durchschnittsmodell. Dieser Wert wird für die Wahrscheinlikeitsberechnung des nächsten Zustandsübergangs aufgenommen. Der aktuelle Zustand wird mit dem Wissen aus der Vergangenheit bestimmt. Das Gesamtsystem nutzt den Output des Kalmanfilters zur Bewegungsabschätzung im Conditional Random Field (CRF), sowie die Nutzung verschiedener Farbwerte innerhalb von definierten Regeln, deren Bedeutung später evaluiert werden können. Das CRF leitet .
\noindent
